...
class PlaidFinancialTool(Component):
    ...
    def _generate_sandbox_access_token(self) -> str:
        ...
        try:
            ...
            self._safe_log(f"Making sandbox_public_token_create request with institution_id: {institution_id}")
            sandbox_response = self.plaid_client.sandbox_public_token_create(sandbox_request)
    ...
    def _get_access_token(self) -> str:
        ...
        if (self.environment == "sandbox" and 
            getattr(self, 'auto_generate_sandbox_token', True)):
    ...
    def _get_accounts(self) -> List[Dict]:
        ...
        try:
            ...
            self._safe_log(f"Retrieved {len(response['accounts'])} accounts from Plaid API")
    ...
    def _get_balances(self) -> List[Dict]:
        ...
        try:
            ...
            self._safe_log(f"Retrieved balances for {len(response['accounts'])} accounts from Plaid API")
            ...
            for account in response['accounts']:
                ...
                self._safe_log(f"Account {account['name']}: {balance_data['current_balance']} {balance_data['currency']}")
            
            self.status = f"Successfully retrieved balances for {len(balances)} accounts"
            self._safe_log(self.status)
            return balances
    ...
    def _get_investments(self) -> Dict:
        ...
        try:
            ...
            self._safe_log(f"Retrieved {len(response['holdings'])} holdings and {len(response['securities'])} securities from Plaid API")
            ...
            for holding in response['holdings']:
                ...
                if not security:
                    self._safe_log(f"Warning: Security not found for holding: {holding['security_id']}")
                    continue
                ...
                self._safe_log(f"Holding {security['name']}: ${market_value:,.2f} {holding_data['currency']}")
    ...
    def _get_summary(self) -> Dict:
        ...
        try:
            ...
            banking_total = sum(
                balance['current_balance'] for balance in balances 
                if balance['account_type'] in ['depository']
            )
            
            credit_total = sum(
                abs(balance['current_balance']) for balance in balances 
                if balance['account_type'] == 'credit'
            )
            
            self._safe_log(f"Banking calculations - Deposits: ${banking_total:,.2f}, Credit: ${credit_total:,.2f}")
            ...
            try:
                ...
                self._safe_log(f"Net worth calculation with investments: ${net_worth:,.2f}")
            except Exception as e:
                ...
                self._safe_log(f"Net worth calculation without investments: ${summary['net_worth']:,.2f}")
            
            self.status = f"Generated financial summary - Net worth: ${summary['net_worth']:,.2f}"
            self._safe_log(self.status)
            return summary
    ...
    def _format_as_text(self, data: Any, data_type: str) -> str:
        ...
        if data_type == "accounts":
            ...
            if data['investments']:
                text += f"Investment Value: ${data['investments']['total_value']:,.2f}\n"
    ...
    def get_financial_data(self, data_type: str = "summary", output_format: str = "text") -> str:
        """
        ...
        """
        self._safe_log(f"get_financial_data() called with data_type='{data_type}', output_format='{output_format}'")
        ...
        try:
            ...
            if data_type not in valid_types:
                self._safe_log(f"Invalid data_type '{data_type}', defaulting to 'summary'")
                data_type = "summary"
            
            # Validate output_format
            valid_formats = ["structured", "json", "text"]
            if output_format not in valid_formats:
                self._safe_log(f"Invalid output_format '{output_format}', defaulting to 'text'")
                output_format = "text"
            
            self._safe_log(f"Processing request for {data_type} data in {output_format} format")
            ...
            self.status = f"Successfully processed {data_type} request ({len(result)} characters)"
            self._safe_log(self.status)
            return result
            ...
            return json.dumps({"error": error_msg, "data_type": data_type, "timestamp": datetime.now().isoformat()}, indent=2)
